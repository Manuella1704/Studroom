...
export default function ChatBox() {
  ...
  const [messages, setMessages] = useState([
    {
      from: "bot",
      text: "Bonjour ! Je suis l’assistant Roomia. Comment puis-je vous aider ?",
    },
  ]);
      ...
      case "home":
        return (
          <div className="flex flex-col h-full p-5 text-sm text-gray-700 bg-white bg-opacity-100 justify-between">
            ...
            <div>
              <h2 className="text-xl font-semibold text-[#2C4A8A] mb-2">
                Poser une question
              </h2>
              <p className="text-gray-600 text-base mb-4">
                Notre bot et notre équipe peuvent vous aider
              </p>
            ...
            <div className="text-center mt-6">
              <p className="text-xs text-gray-400">
                Roomia • Assistance intelligente
              </p>
            </div>
          </div>
        );
      case "chat":
        return (
          <div className="flex flex-col h-full">
            {/* Zone de messages */}
            <div className="flex-1 overflow-y-scroll max-h-[600px] p-5 space-y-3 text-[0.9375rem]">
              {messages.map((msg, i) => (
                <div
                  key={i}
            ...
            <div className="flex items-center border-t p-3">
              <input
                ...
                className="flex-1 px-3.5 py-2.5 border rounded-l-md focus:outline-none text-[0.9375rem]"
              />
              <button
                onClick={handleSend}
                className="bg-[#2C4A8A] text-white px-5 py-2.5 rounded-r-md hover:bg-[#1e3a6a] transition text-[0.9375rem]"
              >
                Envoyer
              </button>
            </div>
  ...
  return (
    <div className="fixed bottom-6 right-6 z-5">
      {!open ? (
        ...
        <div className="w-[600px] h-[780px] bg-white shadow-xl rounded-2xl overflow-hidden border border-gray-200 flex flex-col z-[9999]">
          {/* Header */}
          ...
          <div className="flex justify-around border-t p-2 text-xs text-gray-600">
            <button
              ...
              className={`flex flex-col items-center \${activeTab === 'home' ? 'text-[#2C4A8A]' : ''}`}
            >
              <Home size={16} />
              Accueil
            </button>
            <button
              ...
              className={`flex flex-col items-center \${activeTab === 'chat' ? 'text-[#2C4A8A]' : ''} `}
            >
            ...
            <button
              ...
              className={`flex flex-col items-center \${activeTab === 'help' ? 'text-[#2C4A8A]' : ''}`}
            >
              <HelpCircle size={16} />
              Aide
            </button>
